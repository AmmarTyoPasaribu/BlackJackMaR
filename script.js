...
const resetMoneyButton = document.getElementById("btn-reset-money");
let jumlahuangaslicuy = 5000;
...
resetMoneyButton.addEventListener("click", function () {
  myMoney.textContent = jumlahuangaslicuy;
});

startGameButton.addEventListener("click", function (event) {
  event.preventDefault();
  resultElement.textContent = "";
  const betAmountcuy = parseInt(inputMoney.value);

  if (isNaN(betAmountcuy) || betAmountcuy <= 0) {
    alert("HARUS BERTARUH CUY YAKALI JUDI GK BERTARUH");
    return;
  }
...
takeCardButton.addEventListener("click", function () {
  ...
  if (mySums > 21) {
    ...
    resultElement.textContent = "Lose lu Ayam";
    playLoseSound();
    ...
    holdCardsButton.disabled = true;
    startGameButton.disabled = false;
    resultElement.textContent = "Win Hoki Dek";
    const betAmount = parseInt(inputMoney.value);
    myMoney.textContent = parseInt(myMoney.textContent) + betAmount;
    playWinSound();
      ...
      setTimeout(() => {
        ...
        if (botSums < 18) {
          ...
          if (mySums > 21 || mySums % 22 < botSums % 22) {
            message = "Lose lu Ayam";
            myMoney.textContent -= parseInt(inputMoney.value);
            playLoseSound();
          } else if (botSums > 21 || mySums % 22 > botSums % 22) {
            message = "Win Hoki Dek";
            const betAmount = parseInt(inputMoney.value);
            myMoney.textContent = parseInt(myMoney.textContent) + betAmount;
            playWinSound();
